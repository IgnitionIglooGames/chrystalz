...
public class MapTimeBattleLogic extends AbstractBattle {
    ...
    private void doBattleInternal(final Dungeon bMap, final MapBattle b) {
        ...
        // Create AI Contexts
        this.myContext = new MapAIContext(this.me, bMap);
        this.enemyContext = new MapAIContext(this.enemy, bMap);
        // Reset
        this.me.resetAll();
        this.enemy.resetAll();
        // Set Action Bars
    ...
    private void setCharacterLocations() {
        final RandomRange randX = new RandomRange(0,
                this.battleMap.getRows() - 1);
        final RandomRange randY = new RandomRange(0,
                this.battleMap.getColumns() - 1);
        int rx, ry;
        // Set Player Location
        if (this.me != null) {
            if (this.me.isActive() && this.me.getTemplate().getX() == -1
                ...
                rx = randX.generate();
                ry = randY.generate();
                AbstractGameObject obj = this.battleMap.getCell(rx, ry, 0,
                        DungeonConstants.LAYER_OBJECT);
                while (obj.isSolidInBattle()) {
                    rx = randX.generate();
                    ry = randY.generate();
                    obj = this.battleMap.getCell(rx, ry, 0,
                            DungeonConstants.LAYER_OBJECT);
                }
                this.me.setX(rx);
                this.me.setY(ry);
                this.battleMap.setCell(this.me, rx, ry, 0,
                        DungeonConstants.LAYER_OBJECT);
            }
        }
        // Set Enemy Location
        if (this.enemy != null) {
            if (this.enemy.isActive() && this.enemy.getTemplate().getX() == -1
                ...
                this.enemy.setX(rx);
                this.enemy.setY(ry);
                this.battleMap.setCell(this.enemy, rx, ry, 0,
                        DungeonConstants.LAYER_OBJECT);
            }
        }
    }
    ...
    public void arrowDone(final BattleCharacter hit) {
        ...
        if (hit != null && !hit.getTemplate().isAlive()) {
            ...
            this.battleMap.setCell(new Empty(), hit.getX(), hit.getY(), 0,
                    DungeonConstants.LAYER_OBJECT);
        }
        // Check result
        ...
        if (next != null && nextGround != null && currGround != null) {
            if (!next.isSolidInBattle()) {
                ...
                active.setSavedObject(
                        m.getCell(px, py, 0, DungeonConstants.LAYER_OBJECT));
    ...
    public void redrawOneBattleSquare(final int x, final int y,
        ...
        this.battleGUI.redrawOneBattleSquare(this.battleMap, x, y, obj3);
    }
        ...
        if (player) {
            if (this.me != null && this.me.isActive()) {
                ...
                if (!active.isAlive()) {
                    ...
                    // Remove character from battle
                    this.battleMap.setCell(new Empty(), this.me.getX(),
                            this.me.getY(), 0, DungeonConstants.LAYER_OBJECT);
                }
            }
        } else {
            if (this.enemy != null && this.enemy.isActive()) {
                ...
                if (!active.isAlive()) {
                    ...
                    active.stripAllEffects();
                    // Remove character from battle
                    this.battleMap.setCell(new Empty(), this.enemy.getX(),
                            this.enemy.getY(), 0,
                            DungeonConstants.LAYER_OBJECT);
                }
            }
